    ...
    if($detail_status == '1' && $row_detail['sign_status'] == '0'){
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'], $filePath_signed)) {
            ...
            if($rowdetail_check['sign_status'] == '0'){
                ...
                $sql_signmove_remark = "SELECT move_id FROM sign_move WHERE main_id = '$_POST[main_id]' 
                    AND activity = 'Remark' AND move_status = '1'";
                $res_signmove_remark = mysqli_query($mysqli, $sql_signmove_remark);
                $row_signmove_remark = mysqli_num_rows($res_signmove_remark);

                if($num_row_not_sign >= 1 || $row_signmove_remark >= 1){// หากยังเซ็นไฟล์ไม่ครบ หรือมีการยกเลิกบางรายการ
                    $main_status = '1';
                    $sequ_signed_status = 'W';
                } 
                else{ //เกษียณ หรืออนุมัติเสร็จแล้ว
                    $main_status = '2';
            ...
            }//end detail check sign_status=0
