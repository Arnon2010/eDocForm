...
if($otherDept == '2')
    ...
    $condtion .= "AND depart_id = '$departIdCore' AND depart_cancel = '0'"; //กรณีที่เป็นหน่วยงานหลัก
}    
    ...
    $objsql2 = "SELECT depart_id, depart_name, depart_parent 
        ...
        WHERE depart_id NOT IN
            (
                SELECT depart_id
                FROM edoc_sent
                WHERE edoc_id='$edocId' AND depart_id <> '$objdata[depart_id]'
            )
        AND univ_id = '$univId'  
        ...
        $objsql3 = "SELECT depart_id, depart_name, depart_parent 
            ...
            WHERE depart_id NOT IN
                (
                    SELECT depart_id
                    FROM edoc_sent
                    WHERE edoc_id='$edocId' AND depart_id <> '$objdata2[depart_id]'
                )
            AND univ_id = '$univId'
            ...
            $objsql4 = "SELECT depart_id, depart_name, depart_parent 
                ...
                WHERE depart_id NOT IN
                    (
                        SELECT depart_id
                        FROM edoc_sent
                        WHERE edoc_id='$edocId' AND depart_id <> '$objdata3[depart_id]'
                    )
                AND univ_id = '$univId'
                ...
                $objsql5 = "SELECT depart_id, depart_name, depart_parent 
                    ...
                    WHERE depart_id NOT IN
                        (
                            SELECT depart_id
                            FROM edoc_sent
                            WHERE edoc_id='$edocId' AND depart_id <> '$objdata4[depart_id]'
                        )
                    AND univ_id = '$univId'
