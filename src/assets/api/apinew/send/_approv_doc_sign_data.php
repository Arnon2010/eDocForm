...
while($objdata = mysqli_fetch_assoc($objrs)){ $i++;
    ...
    */
    ...
    $row_sequence = mysqli_fetch_assoc(mysqli_query($con,"SELECT sequ_transfer 
        FROM sign_sequence 
        WHERE main_id ='$mainId'
        AND tposition_id = '$objdata[tposition_id]'
        AND sequ_status = '1' 
    ...
    if($row_sequence['sequ_transfer'] == '0'){
        ...
        $row_move = mysqli_fetch_assoc(mysqli_query($con,"SELECT count(move_id) as numrow_move 
            FROM sign_move
            WHERE main_id ='$mainId' 
            AND tposition_id = '$objdata[tposition_id]'
            AND activity = 'Signatured'
            AND move_status = '1' 
            ")
        );

        if($row_detail['numrow_move'] == 1)
            $add_dismiss = true;
    }
    ...
    while($row_detail = mysqli_fetch_assoc($objrs_detail)){ $k++;
        ...
        */
        ...
        //move status
        $objsql_move = "SELECT move_status FROM sign_move 
            WHERE main_id = '$objdata[main_id]' 
            AND detail_id = '$row_detail[detail_id]' 
            AND tposition_id = '$objdata[tposition_id]'";
        $objres_move = mysqli_query($con, $objsql_move);
        $objdata_move = mysqli_fetch_array($objres_move);

        if($row_detail['remark'] == '' || $row_detail['remark'] == 'null') {
            $remark = '-';
        } else {
        ...
        $data_detail[] = array(
            ...
            'moveStatus'=>$objdata_move['move_status'],
        );
